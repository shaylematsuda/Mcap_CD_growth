...
ggplot(df.mort.noT0, aes(x = time.point, y = mortality, color = dom, fill=dom)) +
  facet_wrap(~light.level, nrow = 1) +
    geom_boxplot()




df.mort.noT0 <- df.mort.noT0 %>%
...
mod <- glm(mortality/100 ~ dli * dom + time.point, family = "quasibinomial", data = df.mort.noT0)
Anova(mod)

mod <- glm(mortality/100 ~ dli * dom, family = "quasibinomial", data = filter(df.mort.T2, !frag.id %in% c("2", "6")))
Anova(mod)
