...
grres.f %>% 
  left_join(df) %>%
  filter(mortality <= 2 & !is.na(mortality)) %>%
...
mod <- lmer(growth.rate ~ dom * light.level + time.point + (1|genotype) + (1|frag.id) + (1|tank), 
            data = filter(left_join(grres.f, df), mortality <= 2 & !is.na(mortality)))
Anova(mod)
...
# Are multiple comparisons being adjusted for correctly or too conservatively?
...
## 
Ds  <- df %>%
  filter(dom == "D", light.level == "Natural")
```
