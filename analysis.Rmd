...
df.mort$treatment<-as.factor(df.mort$treatment)
df.mort$dom<-as.factor(df.mort$dom)


# df.mort<-df.mort%>%      #NO LONGER ANY MISSING LIGHT OR TIMES
#   drop_na(light.level) %>% #drop the UNKNOWN light levels
#   drop_na(time.point) 

#write.csv(df.mort,"dfmort_20220711.csv")

# if missing T0 data add back in with metadata
 df.mort<-df.mort %>%
   add_row(frag.id="134", days=0, time.point="T0",treatment="Zero", light.level="Natural", genotype="878", big.tank="16", tank="9", depth.m=2.4, dom ="D", mortality=0)%>%
    add_row(frag.id="135", days=0, time.point="T0",treatment="One", light.level="Low Shade", genotype="878", big.tank="13", tank="2", depth.m=2.4, dom ="D", mortality=0)%>%
    add_row(frag.id="137", days=0, time.point="T0",treatment="Three", light.level="High Shade", genotype="878", big.tank="17", tank="16", depth.m=2.4, dom ="D", mortality=0)%>%
    add_row(frag.id="195", days=0, time.point="T0",treatment="One", light.level="Low Shade", genotype="858", big.tank="13", tank="2", depth.m=2.4, dom ="C", mortality=0)
...
df.mort<-
  ...
  mutate(treatment = case_when(
    is.na(treatment) & frag.id == 276 & time.point=="T2" ~ "Three",
    is.na(treatment) & frag.id == 2 & time.point=="T2" ~ "Zero",
    is.na(treatment) & frag.id == 25 & time.point=="T2" ~ "Three",
    is.na(treatment) & frag.id == 25 & time.point=="T1" ~ "Three",
    TRUE ~ treatment)) %>%
      mutate(light.level = case_when(
    is.na(light.level) & frag.id == 276 & time.point=="T2" ~ "High Shade",
    is.na(light.level) & frag.id == 2 & time.point=="T2" ~ "Natural",
    is.na(light.level) & frag.id == 25 & time.point=="T2" ~ "High Shade",
    is.na(light.level) & frag.id == 25 & time.point=="T1" ~ "High Shade",
    TRUE ~ light.level)) %>%
   mutate(genotype = case_when(
    is.na(genotype) & frag.id == 276 & time.point=="T2" ~ "900",
    is.na(genotype) & frag.id == 2 & time.point=="T2" ~ "852",
    is.na(genotype) & frag.id == 25 & time.point=="T2" ~ "880",
    is.na(genotype) & frag.id == 25 & time.point=="T1" ~ "880",
    TRUE ~ genotype))%>%
   mutate(big.tank = case_when(
    is.na(big.tank) & frag.id == 276 & time.point=="T2" ~ "13",
    is.na(big.tank) & frag.id == 2 & time.point=="T2" ~ "16",
    is.na(big.tank) & frag.id == 25 & time.point=="T2" ~ "14",
    is.na(big.tank) & frag.id == 25 & time.point=="T1" ~ "14",
    TRUE ~ big.tank))%>%
   mutate(tank = case_when(
    is.na(tank) & frag.id == 276 & time.point=="T2" ~ "3",
    is.na(tank) & frag.id == 2 & time.point=="T2" ~ "9",
    is.na(tank) & frag.id == 25 & time.point=="T2" ~ "7",
    is.na(tank) & frag.id == 25 & time.point=="T1" ~ "7",
    TRUE ~ tank))%>%
   mutate(dom = case_when(
    is.na(dom) & frag.id == 276 & time.point=="T2" ~ "C",
    is.na(dom) & frag.id == 2 & time.point=="T2" ~ "D",
    is.na(dom) & frag.id == 25 & time.point=="T2" ~ "D",
    is.na(dom) & frag.id == 25 & time.point=="T1" ~ "D",
    TRUE ~ dom))
...
df.mort %>%
  pivot_wider(id_cols = c(frag.id, genotype, light.level, dom, time.point),
