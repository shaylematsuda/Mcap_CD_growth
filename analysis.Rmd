...
df <- read_csv("all_data_cleaned.csv") %>% 
  drop_na(mortality) %>%      #remove df with no mortality score
...
# Growth
*** if we filter growth by mortality, we need to figure out what to do with NAs
...
# Run stats on unfiltered dataset
mod <- lmer(growth.rate ~ dom * light.level + (1|genotype) + (1|tank), 
            data = filter(gr,  time.point == "T1"))
Anova(mod)
...
mod <- lmer(growth.rate ~ dom * light.level * time.point + (1|genotype) + (1|tank), 
            data = grf)
Anova(mod)
hist(residuals(mod))

emm <- emmeans(mod, ~ dom*light.level|time.point, adjust = "Tukey")
cld(emm)
