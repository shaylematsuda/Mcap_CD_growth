...
metadata <- metadata %>% select(-treatment, -tratment)
```
BW: original code HP
...
# Join qPCR data with metadata
qpcr <- full_join(qpcr, metadata)
qpcr %>% filter(is.na(genotype)) %>% # 39 frags with qPCR data but no associated metadata (genotype, etc.).... drop these
  print(n = nrow(.))

df <- full_join(BW.data, qpcr)
qpcr %>% filter(is.na(genotype))

BW.data %>% filter(frag.id == 207)
metadata %>% filter(frag.id == 207)
