...
mort2<-pivot_longer(mort2, cols = c(T1, T2), names_to = "time.point", values_to = "mortality")

df <- full_join(df, mort2) # we are missing some values. 
df <- df %>%
  mutate(mortality = if_else(time.point == "T0", 0, mortality)) # mortality at T0 is 0

df %>% filter(is.na(dom)) #what frags do not have dom symb data: 2, 25, 276
...
#add rows for T0 if not already included
 df<-df %>%
   add_row(frag.id="134", days=0, time.point="T0", light.level="Natural", genotype="878", big.tank=16, tank=9, dom ="D", mortality=0)%>%
    add_row(frag.id="135", days=0, time.point="T0", light.level="Low Shade", genotype="878", big.tank=13, tank=9, dom ="D", mortality=0)%>%
    add_row(frag.id="137", days=0, time.point="T0", light.level="High Shade", genotype="878", big.tank=17, tank=26,  dom ="D", mortality=0)%>%
    add_row(frag.id="195", days=0, time.point="T0", light.level="Low Shade", genotype="858", big.tank=13, tank=2, dom ="C", mortality=0)

# Frags missing mortality data? None!
df %>%
  group_by(frag.id) %>%
...
df  <- df %>% 
  group_by(frag.id) %>%
  fill(genotype, light.level, big.tank, tank, depth.m, dom, color, treatment) %>%
  ungroup()
