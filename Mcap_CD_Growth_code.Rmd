...
# Color
```{r}
color <- read_csv("color.score.csv") %>%
  rename(genotype = "Colony #") %>%
  select(-7) %>%
  pivot_longer(2:6, names_to = "obs") %>%
  group_by(genotype) %>%
  drop_na(value) %>%
  count(orange = (value == "O")) %>%
  mutate(conf = n/sum(n),
         color = if_else(orange, "orange", "brown")) %>%
  filter(conf == max(conf)) %>%
  select(genotype, color, conf) %>%
  mutate(genotype = factor(genotype))
  
df <- full_join(df, color)

# some genotypes have no color associated with them
df %>%
  filter(is.na(color)) %>%
  distinct(genotype) %>%
  arrange(genotype)
...
df  <- df %>% 
  group_by(frag.id) %>%
  fill(genotype, light.level, big.tank, tank, depth.m, dom, color) %>%
  ungroup()
```
...
# mortality (factor scores...delete when we switch to percent above)
```{r}
 
morttest<-read_csv("UROP_mortality.csv")
mort$frag.id <- as.factor(as.character(mort$frag.id))

mort <- as_tibble(mort)
mort<-drop_na(mort)
mort2<-pivot_wider(mort, names_from = time.point, values_from = mortality, )
...
df <- full_join(df, mort2) # we are missing some values
df <- df %>%
  mutate(mortality = if_else(time.point == "T0", 1, mortality))

# # some frags have mort data, but nothing else...? FIXED SM 1/28/22
# df %>% filter(is.na(dom))
