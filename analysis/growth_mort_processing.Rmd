...
This is the data wrangling script for the "Symbiont-mediated tradeoffs between growth and heat tolerance are modulated by light and temperature in the coral Montipora capitata" manuscript.  
...
# Genotype 864 
# 864 screening biopsy was 90% D. Genotype assigned D. 
df %>%
...
# screening biopsy for '867' was D.

# Genotype 868 = Two frags of this genotype, one D and one C
qpcr_raw868<-qpcr_raw %>%
          filter(frag.id %in% c("110","112"))
# Two fragments of 112 -> @T0 was D-dom (58% D), 110 only qPCR at T2 (T0 failed) which could have been due to symbiont change over time. The ct values were lower in the 112 fragment, and much later in the 110 fragment, providing more confidence for the D designation. The standard deviation for the D designation was 0.21 whereas it was 1.22 for the C. 
# labeled as D
...
# Label all these genotypes as D's 
genos <- c("864", "867B", "868", "880", "912", "922")
df <- df %>%
  mutate(dom = if_else(genotype %in% genos, "D", dom))

# Check if any genotypes entirely missing symbiont data
df %>%
  group_by(genotype) %>%
  summarize(missingdom = !any(!is.na(dom))) %>%
  filter(missingdom)  

# Parent colony screening biopsy for 894 was C
df <- df %>%
...
#Table Genotype x symbiont ID, qPCR
df1<-df #copy
