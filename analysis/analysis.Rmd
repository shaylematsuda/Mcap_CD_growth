...
# So with the quasibinomial the interaction term is not sig. in anova, but the posthocs show highly sig symbiont effect at low light, but not high light, suggesting there is an interaction... weird...
...
####original fig
fig1 <- ggplot(data = df.mort.T2,
>>>>>>> ee1700f297a0699999833df42d16c3460862d282
       aes(x = dli, y = mortality, color = dom, group = dom)) +
...
# again but with dli and not  light level ###
...
mod<-lmer(mortality_log~dli*dom+(1|genotype), df.mort.T2)
anova(mod)
qqPlot(residuals(mod)) #looks better

#not working with dli as numeric. 
  emm<-emmeans(mod,  ~ dli|dom, adjust="Tukey") #high light different
  cld(emm)
  pairs(emm)  #C dom: High diff from all, D dom: high diff from all.
   emm<-emmeans(mod,  ~dom|dli, adjust="Tukey") #high light different
  cld(emm)
  pairs(emm)  # C is diff from D at all light levels except High light. 


```
#Revision.1 mortality binned into percent groups - how to run this
```{r}
...
# chi <- chisq.test(df.mort.T2$light.level, df.mort.T2$mortcat)
# summary(chi)

ggplot(df.mort.T2, aes(x = light.level, fill = mortcat)) + 
  facet_wrap(~dom) + 
...
#Revision.1 mortality only  3 light treatments (combine dark ones) I THINK THIS IS THE WAY
```{r}
...
# usingdli: residuals not amazing 
mod<-lmer(mortality~dli*dom+(1|genotype), df.mort.T2.Nat.1shade)
anova(mod)
qqPlot(residuals(mod)) #Terrible
...
mod<-lmer(mortality_log~dli*dom+(1|genotype), df.mort.T2.Nat.1shade)
anova(mod)
qqPlot(residuals(mod)) #GOOD
#no post hoc.
