...
#average tissue loss per treatment * dom 
df.mort.T2 %>%
  group_by(light.level,dom) %>%
  dplyr::summarise(avg_loss = mean(mortality, na.rm = TRUE), .groups = "drop")
#median
df.mort.T2 %>%
  group_by(light.level,dom) %>%
  dplyr::summarise(median = median(mortality, na.rm = TRUE), .groups = "drop")
...
```
# sumbission 1 mortality  - Delete before Zenodo
```{r}
...
rbind(contrast(emm, method = "pairwise", by = c("dli", "time.point"))[c(2,4,6,8,9,11,13,15)], adjust = "fdr")
  ...
  #Coral growth rate percent differences between C-D
      #calc percent difference between each set of treatments across 2 months,(Growth C - Growth for D)/Growth for            D]*100%
    mean.growth<-data.frame(emm) #make df from fitted emmeans

    mean.grow.all<-mean.growth %>%
     group_by(dli, dom, time.point) %>%
       summarise(meangr = mean(emmean)) %>%
          pivot_wider(names_from = dom, values_from=meangr)%>%
           mutate(percent=((((abs(C-D))/D))*100))
    
    #growth of all corals across whole experimemt at 4 avg DLIs
    emm <- emmeans(mod, ~ dli, at = list(dli = c(0.36, 0.76, 1.55, 3.80)))

    
# Get model outputs for time.point comparision
...
# Get model outputs for light treatment comparision (not time, no dom)
emm <- emmeans(mod, ~ dli, at = list(dli = sort(unique(gr$dli))))
rbind(contrast(emm, method = "pairwise", by = c( "time.point")), adjust = "fdr")
...
# Calculate the mean and median percent D in C31-n and D glynnii-dom genotypes

qPCR.df.propD<-qPCR.df %>% 
  dplyr::select(-c("frag.id","light.level","big.tank", "tank","Dry.Weigh.g","time.point")) 
qPCR.df.propD<-na.omit(qPCR.df.propD)

qPCR.df.propD
qPCR.df.propD2<-qPCR.df.propD %>%
  group_by(genotype, dom)%>%
   summarise(meanD = mean(propD))
     
qPCR.df.propD2 %>%
  group_by(dom)%>%
    summarise(medD = median(meanD)) 
...
p <- ggplot(qPCR.df.propD2, aes(x=dom, y=meanD)) + 
  geom_boxplot();p

p1<- ggviolin(qPCR.df.propD2, x = "dom", y = "meanD",
   add = "boxplot", fill = "dom")+
  coord_cartesian(ylim = c(-0.1,1.3))+
  theme(legend.position = 'none');p1

ggsave(p1, filename = "output/figure_qPCR.png", width = 174, height = 300, units = "mm")
