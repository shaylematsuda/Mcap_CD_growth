...
loggers <- map_dfr(csv_files,function(csv) {
  ...
  return(
    ...
    read_csv(csv,
             skip = 1, # skip 1 row
            col_skip(1, 4:10),
             col_names = c("datetime","Temp"), # name our columns
             col_types = "cn") %>% # force it to read the columns as numeric, character, character, numeric, numeric ("nccnn"
                                      # note the %>% on the line above. That will "pipe" the csv file into the next function
      # now we trim off whitespace from our dates and times
      # mutate is used to change columns
      # across(where(is.character)) chooses just columns containing character string
...
#melt the df and then add metadata
Mcap.temp.2.melt<-melt(Mcap.temp.2, id.vars = c("datetime")) #melt
  #rename cols
Mcap.temp.2.melt<-rename(Mcap.temp.2.melt, temp.logger = "variable")
Mcap.temp.2.melt<-rename(Mcap.temp.2.melt, temp = "value")

#merge with metadata
Mcap.temp.2.melt.meta<-merge(Mcap.temp.2.melt, logger.df)


temp_plot<-ggplot(data=Mcap.temp.2.melt.meta, aes(x=datetime, y=temp, group = Shade, color=Shade)) +
  # geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
  #               width=0.1, show.legend = F)+
  #geom_point(aes(color=Treatment), size=4, show.legend = F)+
  geom_line(aes(color=Shade), linetype=2, show.legend = F)+
