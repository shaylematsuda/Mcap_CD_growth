...
Mcap.light.2.melt<-melt(Mcap.light.2, id.vars = c("datetime")) #melt
 Mcap.light.2.melt<- tibble(Mcap.light.2.melt)
  #rename cols
Mcap.light.2.melt<-rename(Mcap.light.2.melt, Light.logger = "variable") #WHY ISN"T THIS RUNNING NOW NOTHING CHANGED????
...
# calculate mean light level (low light level could be due to algal overgrowth)
...
aggregate(Light~Shade, data=Mcap.light.2.melt.meta, min),
aggregate(Light~Shade, data=Mcap.light.2.melt.meta, max),
aggregate(Light~Shade, data=Mcap.light.2.melt.meta, mean))

mySummary <- mySummary[,c(1,2,4,6)]
colnames(mySummary) <- c("Shade", "min", "max", "mean")
mySummary

#summary T0-T1
Mcap.light.T1 <- Mcap.light.2.melt.meta %>%
                        filter(datetime >= as.Date('2017-10-29') & datetime <= as.Date('2017-11-29')) #why not working
mySummary <- data.frame(
aggregate(Light~Shade, data=Mcap.light.T1, min),
aggregate(Light~Shade, data=Mcap.light.T1, max),
aggregate(Light~Shade, data=Mcap.light.T1, mean))
...
#summary T1-TF
Mcap.light.TF <- Mcap.light.2.melt.meta %>%
                        filter(datetime >= as.Date('2017-11-30') & datetime <= as.Date('2018-01-08')) #why not working
mySummary <- data.frame(
aggregate(Light~Shade, data=Mcap.light.TF, min),
aggregate(Light~Shade, data=Mcap.light.TF, max),
aggregate(Light~Shade, data=Mcap.light.TF, mean))
