...
loggers_wide <- loggers_wide %>%
                        filter(datetime >= as.Date('2017-10-29') & datetime <= as.Date('2018-01-08')) #
...
# calculate mean light level (low light level could be due to algal overgrowth consider deleting)
# mean.light<- ddply(Mcap.light.2.melt.meta, c("Shade"), summarise, 
#                  N    = length(Light[!is.na(Light)]), 
#                  mean = mean(Light, na.rm=TRUE), 
#                  sd   = sd(Light, na.rm=TRUE),
#                  se   = sd / sqrt(N), 
#                  max = max(Light, na.rm=TRUE) 
# );mean.light
...
#summary T1-TF
Mcap.temp.TF <- Mcap.temp.2.melt.meta %>%
                        filter(datetime >= as.Date('2017-11-30') & datetime <= as.Date('2018-01-08')) #why not working
mySummary.temp <- data.frame(
