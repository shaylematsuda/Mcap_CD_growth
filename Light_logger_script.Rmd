...
logger.df<-read.csv("UROP_logger_locations.csv") #read in logger tank locations
...
#add time point column 
  mod<- lm(Light~Shade*time.point, data=Mcap.light.2.melt.meta)
  anova(mod)
  
  emm <- emmeans(mod, ~ Shade*time.point)
rbind(contrast(emm, method = "pairwise", by = "Light"), adjust = "fdr")
```
...
#DLI model
mod <- lmer(dli ~ Shade*time.point + (1|Tank), 
            data = light_res.dli)
anova(mod)
...
#PAR model
mod <- lmer(Light ~ Shade*time.point + (1|Tank), 
