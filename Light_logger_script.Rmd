...
# subset only > 0 values (daylight values)
Mcap.light.2.melt.meta.DAY<-Mcap.light.2.melt.meta %>%
  filter(Light>=1)

#mean across entire experiment only if we do it this way
Mcap.light.2.melt.meta.DAY %>%
 group_by(Shade) %>%
 summarise(mean=mean(Light), median=(median(Light)), min=(min(Light)))

#summary stats T0-T1
Mcap.light.T1 <- Mcap.light.2.melt.meta.DAY %>%
                        filter(datetime >= as.Date('2017-10-29') & datetime <= as.Date('2017-11-29')) 
 Mcap.light.T1 %>%
  group_by(Shade) %>%
  summarise(mean=mean(Light), median=(median(Light)), max=max(Light), min=min(Light))

  #extract max per day
  Mcap.light.T1$date<-date(Mcap.light.T1$datetime)
  T1.max<-Mcap.light.T1 %>%
    group_by(date,Shade) %>%
    summarise(max_val = max(Light))

  # average daily max by shade treatment
  T1mean<-T1.max%>%
   group_by(Shade)%>%
    summarise(mean=mean(max_val))
    T1mean$time.point<-"T1"

#summary T1-Ts
Mcap.light.T2 <- Mcap.light.2.melt.meta.DAY %>%
                        filter(datetime >= as.Date('2017-11-30') & datetime <= as.Date('2018-01-08')) #why not working
 Mcap.light.T2 %>%
  group_by(Shade) %>%
  summarise(mean=mean(Light), median=(median(Light)), max=max(Light), min=min(Light))

  #extract max per day
  Mcap.light.T2$date<-date(Mcap.light.T2$datetime)
  T2.max<-Mcap.light.T2 %>%
    group_by(date,Shade) %>%
    summarise(max_val = max(Light))
  ...
  T2mean<-T2.max%>%
   group_by(Shade)%>%
    summarise(mean=mean(max_val))
    T2mean$time.point<-"T2"

#combine for ROSS
Light.by.time<- left_join(T1mean, T2mean, by="Shade")
```
...
#remove data when loggers pulled (same as light)
loggers_wide<-loggers_wide%>%
     filter(!(date == '2017-11-01'))%>%
     filter(!(date == '2017-11-18'))%>%
     filter(!(date == '2017-11-29'))%>%
...
temp_plot<-ggplot(data=Mcap.temp.meta.melt, aes(x=datetime, y=temp, group = Big.tank, color=Big.tank)) +
  # geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
  #               width=0.1, show.legend = F)+
  #geom_point(aes(color=Treatment), size=4, show.legend = F)+
  geom_line(aes(color=Big.tank), linetype=2, show.legend = F)+
  #scale_color_manual(values=c("dodgerblue3", "darkred"))+
  xlab("") + #Label the X Axis
  #ylim(0, 11) + #set Y limits
  #scale_y_log10()+
...
Mcap.temp.meta.melt$temp<-as.numeric(Mcap.temp.meta.melt$temp)
#Summary across entire experiment
mySummary.temp <- data.frame(
aggregate(temp~Shade, data=Mcap.temp.meta.melt, max),
aggregate(temp~Shade, data=Mcap.temp.meta.melt, mean))

mySummary.temp <- mySummary.temp[,c(1,2,4,6)]
colnames(mySummary.temp) <- c("Shade", "min", "max", "mean")
mySummary.temp

#summary T0-T1   NOT WORKING, not splicing by shade treatment....
Mcap.temp.T1 <- Mcap.temp.meta.melt %>%
                        filter(datetime >= as.Date('2017-10-29') & datetime <= as.Date('2017-11-29')) #why not working
mySummary.temp <- data.frame(
aggregate(temp~Shade, data=Mcap.temp.T1, min),
aggregate(temp~Shade, data=Mcap.temp.T1, max),
aggregate(temp~Shade, data=Mcap.temp.T1, mean))
...
Mcap.temp.TF <- Mcap.temp.meta.melt %>%
                        filter(datetime >= as.Date('2017-11-30') & datetime <= as.Date('2018-01-07')) #why not working
mySummary.temp <- data.frame(
aggregate(temp~Shade, data=Mcap.temp.TF, min),
aggregate(temp~Shade, data=Mcap.temp.TF, max),
aggregate(temp~Shade, data=Mcap.temp.TF, mean))

mySummary.temp <- mySummary.temp[,c(1,2,4,6,8)]
colnames(mySummary.temp) <- c("Shade", "min", "max", "mean")
mySummary.temp  
...
#try again
#mean across entire experiment only if we do it this way
Mcap.temp.meta.melt2<-Mcap.temp.meta.melt #make copy
Mcap.temp.meta.melt2<-na.omit(Mcap.temp.meta.melt2) #remove NAs





Mcap.temp.meta.melt2 %>%
 group_by(Big.tank) %>%
 summarise(mean=mean(temp), median=(median(temp)), min=(min(temp)))
